...
public class UDPMessageReceiver extends Thread implements LoggingServer {
    ...
    private LinkedBlockingQueue<Message> zookeeperQ;
    private LinkedBlockingQueue<Message> gossipQ;
    private Logger logger;
    private ZooKeeperPeerServer peerServer;

    public UDPMessageReceiver(LinkedBlockingQueue<Message> zookeeperQ, LinkedBlockingQueue<Message> gossipQ, InetSocketAddress myAddress, int myPort, ZooKeeperPeerServer peerServer) throws IOException {
        this.zookeeperQ = zookeeperQ;
        this.gossipQ = gossipQ; 
    ...
    public void run() {
        ...
        while (!this.isInterrupted()) {
            try {
                ...
                this.logger.fine("Is this message considered a Stray? " + this.strayElectionMessage(received)); 
                ...
                //end stage 5 logic
                    ...
                    while(!done){
                        if(received.getMessageType() == MessageType.ELECTION) {
                            done = this.zookeeperQ.offer(received);
                            this.logger.fine("added ELECTION to q"); 
                        }
                        else done = this.gossipQ.offer(received); 
                    }
                }
            }
    ...
    private boolean strayElectionMessage(Message received) {
        ...
        ElectionNotification receivedNotification = ZooKeeperLeaderElection.getNotificationFromMessage(received);
        if (receivedNotification.getState() != ZooKeeperPeerServer.ServerState.LOOKING && this.peerServer.getCurrentLeader() != null) { //what is this saying? 
            return true;
        }
        else {
            return false;
